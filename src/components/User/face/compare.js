...
export default function Compare() {
  ...
  const imagep = searchParams.get('image');
  const image  = `/images/target/${imagep}`;
  ...
  const loadModels = useCallback(async () => {
    ...
    try {
      ...
      if (!image || isUnmounted.current) return;
  ...
  }, [image, updateState, setModelState]);
  ...
  useEffect(() => {
    if (!name || !image) {
      updateState({
        errorMessage: 'Missing required URL parameters (name, or image)',
        detectionState: 'error'
      });
    }
  }, [name, image, updateState]);
